...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const signIn = async (email: string, password: string) => {
    ...
    try {
      ...
      const deletionInfo = await AuthService.isDeactivated(user.id);

      if (deletionInfo?.deletion_requested && deletionInfo?.deletion_requested_at) {
      // Calculate days remaining
      const deletionDate = new Date(deletionInfo.deletion_requested_at);
      const daysRemaining = Math.max(
        0,
        30 - Math.floor((Date.now() - deletionDate.getTime()) / (1000 * 60 * 60 * 24))
      );

      return {
        ...result,
        accountPendingDeletion: true,
        deletion_requested_at: deletionInfo.deletion_requested_at,
        daysRemaining,
      };
    }
  ...
  const restoreAccount = async (userId : string) => {
    ...
    try {
      ...
      if (!idToUse) throw new Error("No user is currently signed in");

      await AuthService.restoreAccount(idToUse);
