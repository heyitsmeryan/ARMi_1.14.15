...
class AuthServiceClass {
  ...
  async isDeactivated(userId: string): Promise<{
    ...
    const { data, error } = await AuthService.supabase
      .from('user_profiles')
      .select('deletion_requested, deletion_requested_at')
      .eq('user_id', userId)
      .single();
  ...
  async deleteAccount(userId: string) {
    ...
    try {
      ...
      if (updateError) throw new Error(updateError.message);
        ...
        console.error('Failed to mark the account for deletion: ',error);
        throw error;
    }
  }
  ...
  async restoreAccount(userId: string) {
    ...
    try {
      ...
      if (updateError) throw new Error(updateError.message);
        ...
        console.error('Failed to unmark the account for deletion: ',error);
        throw error;
    }
  }
